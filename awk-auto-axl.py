...
dir = '/run/media/maryamma/One Touch/project_md/axl3/5.2/double-after25us/equilibrium/20us/neighbor/'
...
# dir = "/run/media/maryamma/One Touch/project_md/project_2/gromacs/step7/prot-const/neighboring-50ns/"
...
for first_folder in first_folder_list:
    if not first_folder.startswith('chol'):
        ...
        # second_folder_list = ['all_not_chol']
        for second_folder in second_folder_list:
            if not second_folder.startswith('chol') and not second_folder.startswith('all-'):
                try:
                    print(second_folder)
